...
from segmentation_models_pytorch import utils as utils_smp
...
def main(CFG, data_path, batch_size, with_val=False):
    ...
    val_dataset = vessel_dataset(data_path, mode="validation")
    val_loader = DataLoader(
            val_dataset, batch_size, shuffle=False, num_workers=16, pin_memory=True, drop_last=False)
    ...
    model = smp.Unet(backbone, in_channels=1, classes=1)
    logger.info(f'\n{model}\n')
    loss = get_instance(losses, 'loss', CFG)
    ###loss = utils_smp.losses.DiceLoss()
    trainer = Trainer(
        model=model,
        loss=loss,
        CFG=CFG,
