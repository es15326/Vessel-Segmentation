...
def data_process(data_path, name, patch_size, stride, mode):
    ...
    if mode == "training":
        ...
        save_patches(img_patch, gt_patch, save_path, "gt_patch", name)
...
def save_patches(imgs_list, gt_list, path, type, name):
    for i, sub in enumerate(imgs_list):
        if not torch.all(gt_list[i] == 0):
            with open(file=os.path.join(path, f'img_patch_{i}.pkl'), mode='wb') as file:
                pickle.dump(np.array(imgs_list[i]), file)
            with open(file=os.path.join(path, f'gt_patch_{i}.pkl'), mode='wb') as file:
                pickle.dump(np.array(gt_list[i]), file)
