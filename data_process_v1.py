...
def data_process(data_path, name, patch_size, stride, mode, json_file_path=''):
    ...
    if name == "DRIVE":
        ...
        img_path = os.path.join(data_path, temp_mode, "images")
        gt_path = os.path.join(data_path, temp_mode, "1st_manual")
        file_list = list(sorted(os.listdir(img_path)))
        if mode == "training" or mode == "validation":
            json_file_path = os.path.join(data_path, json_file_path)
            data = load_data_from_json(json_file_path)
            file_list = extract_images_and_labels(data)
    elif name == "CHASEDB1":
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--stride', default=4,
    ...
    data_process(args.dataset_path, args.dataset_name, args.patch_size, args.stride, "validation", 'cross_validation/val_data_1.json')

    data_process(args.dataset_path, args.dataset_name, args.patch_size, args.stride, "training", 'cross_validation/train_data_1.json')
    data_process(args.dataset_path, args.dataset_name, args.patch_size, args.stride, "test")
